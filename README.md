...
## 🖧 התקנה רשתית (כמה עמדות מול שרת אחד)
...
2. ערוך את `start-hbsearch.bat`: `DATAROOT` (נתיב הנתונים בשרת) ו-`CLIENTBASE` (תיקיית הבסיס המשותפת ברשת, UNC — למשל `\\192.168.1.20\f\HebrewBooks`).
3. הרשאות חד-פעמיות (כ-Administrator): `netsh http add urlacl url=http://+:8080/ user=Everyone` ופתיחת פורט 8080 בחומת-האש.
4. הרץ את `start-hbsearch.bat`.

**בכל עמדה (בלי להקליד כלום):**
פתח דפדפן וגלוש ל-`http://<כתובת-השרת>:8080` → לחץ **"התקן עכשיו"** → פתח את הקובץ שירד. התוכנה תותקן ותוגדר אוטומטית מול השרת.
